...
// updated code on 3/5 10:40pm
  ...
  Ray new_ray = Ray(new_ray_o, new_ray_d);
  new_ray.depth = (int)(r.depth - 1);
  Spectrum income_radiance = trace_ray(new_ray, isect.bsdf->is_delta());
  // return income_radiance * sp * cos_theta(w_in) / (pdf * (1 - sp.illum()));
  // return income_radiance * sp * cos_theta(w_in) / (pdf * ref);
  return income_radiance * sp * abs_cos_theta(w_in) / (pdf * ref);
  // pdf * ref   for proj3-2

  // also try this with clamp + no r.max_t
  // update: 1 - ref doesn't work
  // return income_radiance * sp * cos_theta(w_in) / (pdf * (1 - ref));
...
Spectrum PathTracer::trace_ray(const Ray &r, bool includeLe) {
  ...
  // only for rendering indirect lighting
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  if (num_samples == 1) {
    ...
    // // for part 4
    // Vector2D random_2d = gridSampler->get_sample();
    // Ray r = camera->generate_ray_for_thin_lens((((double)x + 0.5) / (double)sampleBuffer.w), (((double)y + 0.5) / (double)sampleBuffer.h), random_2d.x, random_2d.y);
    ...
    for (int i = 0; i < num_samples; i++) {
      ...
      // // for part4
      // random_2d = gridSampler->get_sample();
      // Ray rr = camera->generate_ray_for_thin_lens(sp.x, sp.y, random_2d.x, random_2d.y);
