...
Ray Camera::generate_ray(double x, double y) const {
  ...
  double X = bottom_left.x + (top_right.x - bottom_left.x) * x;
  double Y = bottom_left.y + (top_right.y - bottom_left.y) * y;
...
Ray Camera::generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta) const {
  ...
  // Vector3D len = Vector3D(lensRadius * sqrt(rndR) * cos(2 * PI * rndTheta), lensRadius * sqrt(rndR) * sin(2 * PI * rndTheta), 0.0);
  ...
  // double X = top_right.x - bottom_left.x;
  // double Y = top_right.y - bottom_left.y;

  // Vector3D d = Vector3D(bottom_left.x + X * x, bottom_left.y + Y * y, -1.0);
  // Ray red(o, d);

  // Vector3D pfocus = focalDistance * red.d;
  // Vector3D dir = pfocus - len;
  // Ray blue(len, dir);

  // blue.d.normalize();

  // Vector3D new_dir = c2w * blue.d;
  // Vector3D new_ori = c2w * blue.o;

  // Ray output(new_ori + pos, new_dir, fClip);
  // output.min_t = nClip;
